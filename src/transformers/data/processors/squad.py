...
from .convert_text import get_text_num, get_text_num_with_answer,get_text_num_with_answers
...
def _improve_answer_span(doc_tokens, input_start, input_end, tokenizer, orig_answer_text):
    ...
    tokenize = lambda s: split_digits(tokenizer.tokenize(s))
...
def bionumfactqa_convert_example_to_features(example, max_seq_length, doc_stride, max_query_length, is_training):
    features = []
    if is_training and not example.is_impossible:
        # Get start and end position
        start_position = example.start_position
    ...
    while len(spans) * doc_stride < len(all_doc_tokens):
        ...
        doc_num_end = np.digitize(len(spans) * doc_stride + paragraph_len , doc_num_indices)
...
def bionumfactqa_convert_examples_to_features(
    examples,
    tokenizer,
    max_seq_length,
    doc_stride,
    ...
    with Pool(threads, initializer=squad_convert_example_to_features_init, initargs=(tokenizer,)) as p:
        annotate_ = partial(
            bionumfactqa_convert_example_to_features,
            max_seq_length=max_seq_length,
            doc_stride=doc_stride,
            max_query_length=max_query_length,
            is_training=is_training,
    ...
    if return_dataset == "pt":
        ...
        all_p_mask = torch.tensor([f.p_mask for f in features], dtype=torch.float)
        all_is_impossible = torch.tensor([f.is_impossible for f in features], dtype=torch.float)
        all_number = torch.tensor([f.number for f in features], dtype=torch.float)

        if not is_training:
            all_feature_index = torch.arange(all_input_ids.size(0), dtype=torch.long)
            dataset = TensorDataset(
                all_input_ids, all_attention_masks, all_token_type_ids, all_feature_index, all_cls_index, all_p_mask, all_number
            )
        else:
            all_start_positions = torch.tensor([f.start_position for f in features], dtype=torch.long)
            all_end_positions = torch.tensor([f.end_position for f in features], dtype=torch.long)
...
class BioNumFactQAProcessor(SquadProcessor):
    train_file = "train-v1.1.json"
    dev_file = "dev-v1.1.json"
...
class BioNumFactQAExample(object):
    def __init__(
        self,
        qas_id,
        question_text,
        ...
        if answer_text is not None:
            ...
            self.context_text, self.context_nums, new_answers = get_text_num_with_answers(context_text, answers)
...
class BioNumFactQAFeatures(object):
    """
